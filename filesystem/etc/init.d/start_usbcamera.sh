...
/etc/init.d/usb_gadget_configfs.sh rndis+uvc_bulk_7_noraw+hid_arto 0 0 0x1d6b 0x0102

if [ -e "usrdata/apps/uvc_service2" ]; then
	...
	cp -f /usrdata/apps/uvc_service2 /tmp/uvc_service
	/tmp/uvc_service --mode uvc --fps 30 --res0 1920 1080 --res1 640 480 --osd -b &
fi

if [ -e "/local/usr/bin/test_hid_service" ]; then
	cp -f /usrdata/apps/test_hid_service /tmp/test_hid_service
	/tmp/test_hid_service &
fi
