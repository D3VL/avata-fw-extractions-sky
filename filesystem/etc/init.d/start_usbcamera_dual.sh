...
/etc/init.d/usb_gadget_configfs.sh uvc_bulk_7_noraw+uvc_bulk_7_hdr+hid_arto 0 0 0x1d6b 0x0102

if [ -e "/local/usr/bin/uvc_service2" ]; then
	echo "start uvc server"
	/local/usr/bin/uvc_service2 --mode uvc --fps 25 --ctl-int 2 --str-int 3 --pipe_index 1 --multi --res0 1920 1080 --res1 640 480 --osd --hdr -b &
	/local/usr/bin/uvc_service2 --mode uvc --fps 30 --ctl-int 0 --str-int 1 --pipe_index 0 --multi --res0 1920 1080 --res1 640 480 -b &
fi
